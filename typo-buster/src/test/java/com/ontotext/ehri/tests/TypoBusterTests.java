...
public class TypoBusterTests {
    private static final String DOCUMENT_PATH = "src/test/resources/supernova.txt";
    private static final String DOCUMENT_ENCODING = "UTF-8";
    private static final String RULES_PATH = "src/test/resources/rules/DefaultTokeniser.rules";
    private static final String RULES_ENCODING = "UTF-8";
    ...
    public void test() {
        ...
        try {
            ...
            File rulesFile = new File(RULES_PATH);
            SimpleTokeniser tokeniser = new SimpleTokeniser();
            tokeniser.setRulesURL(rulesFile.toURI().toURL());
            tokeniser.setEncoding(RULES_ENCODING);
            tokeniser.setDocument(document);
            tokeniser.init();
            tokeniser.execute();

            // learn typos
            File modelFile = new File("src/test/resources/model.gz");
            ...
            Annotation supernova = document.getAnnotations().get(7);
            ...
            Annotation suprenova = document.getAnnotations().get(1198);
            ...
            Annotation gastronomers = document.getAnnotations().get(1214);
